rldata = open("C:\\users\\quent\\Desktop\\AoC2021\\AoC15.txt", "r").read().split()
...
def getPathLength(r):
    op = {
        (0, 0): (0, est_len(0, 0, 0))
    }  # open dictionary of squares whose neighbours are to be investigated
    ...
    while (width * r - 1, height * r - 1) not in cl:
        ...
        x, y = min(
            op, key=lambda a: op[a][1]
        )  # can speed by sorting list - but less readable
        d, e = op[x, y]
        cl[x, y] = d, e
        del op[x, y]
